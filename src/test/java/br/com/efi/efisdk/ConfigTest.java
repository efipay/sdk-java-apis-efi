...
public class ConfigTest {
	...
	public void shouldBuildOptionsSuccessfully() throws Exception
	{
		...
		options.put("url", "consultoria-webhook-h.sejaefi.com.br");
		...
		try {
			...
			assertTrue(config.get("baseUri").equals("consultoria-webhook-h.sejaefi.com.br"));
	...
	public void shouldBuildOptionsWithoutUrl() throws Exception
	{
		...
		when(urlSet.has("production")).thenReturn(true);
		when(urlSet.has("sandbox")).thenReturn(true);
		when(urlSet.getString("production")).thenReturn("https://cobrancas.api.efipay.com.br");
		when(urlSet.getString("sandbox")).thenReturn("https://cobrancas-h.api.efipay.com.br");
		when(configurations.get("URL")).thenReturn(urlSet);
		try {
			Config configuration = new Config(options, configurations);
			...
			assertTrue(config.get("baseUri").equals("https://cobrancas-h.api.efipay.com.br"));
		} catch (IOException e) {
			fail("The file config.json doesn't exist or is not in the right fouder.");
		}
	}
